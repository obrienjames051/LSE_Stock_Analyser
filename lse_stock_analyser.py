...
# MAIN
# ─────────────────────────────────────────────────────────────────────────────
def ask_for_mode() -> bool:
    """
    Ask whether to run in Live mode (saves to CSV) or Preview mode (no saving).
    Returns True if Live mode, False if Preview mode.
    """
    console.print(
        "\n[bold cyan]Run mode[/bold cyan]\n"
        "[dim]  [L] Live mode    — results are saved to the CSV log (use once a week)\n"
        "  [P] Preview mode — results are shown but nothing is saved[/dim]\n"
    )
    while True:
        raw = input("  Choose mode (L / P): ").strip().upper()
        if raw in ("L", "LIVE"):
            return True
        elif raw in ("P", "PREVIEW"):
            return False
        else:
            console.print("  [red]Please enter L for Live or P for Preview[/red]")
